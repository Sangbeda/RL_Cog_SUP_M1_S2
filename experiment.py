...
def run_experiment(exp_params: dict, environment: Environment, agent: RLagent, replay_threshold=None) -> dict:
    ...
    for episode_idx in range(exp_params['nr_of_episodes']):
        ...
        while not terminated and nr_of_steps < exp_params['max_steps']:
            # 1) Check to see what we can do and pick an action to follow
            action = agent.action_selection()
            action = action['action']
            ...
            if replay_threshold is not None and abs(delta_Q) > replay_threshold and agent._replay_type != 'bidirectional':
                agent.memory_replay()
            elif agent._replay_type == 'bidirectional':
                agent.memory_replay()
...
def run_experiment_adaptive(exp_params: dict, environment: Environment, agent: RLagent, replay_threshold=None) -> dict:
    """
    Simply runs the RL experiment as specified by the parameters.
    :param exp_params: further parameters of the experiment
    :param environment: the environment we're using
    ...
    # Running the experiment ###########################################################################################
    reward_rate = [0] * exp_params['nr_of_episodes']
    episode_to_change_reward_location = exp_params['abruptive_episode']
    if episode_to_change_reward_location > exp_params['nr_of_episodes']:
        raise ValueError("Episode to change reward location exceeds the number of episodes.")
    for episode_idx in range(exp_params['nr_of_episodes']):
        terminated = False
        nr_of_steps = 0
        ...
        if episode_idx == episode_to_change_reward_location-1:
            environment._change_reward_location()
